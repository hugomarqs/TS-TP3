...
const title = "Files Monitor";

const DEFAULT_EMAIL_TEMPLATE = `To: [email]
From: [from]
Subject: Files Monitor for [name]: [file]
Importance: high
Monitor Name: [name]
Date/Time: [date]
Hostname: [hostname]
...
const DEFAULT_SMS_TEMPLATE = `Files Monitor: [name]: [file]
[lines]
`;
...
const app = {
	...
	checkAll: function() {
		...
		async.eachSeries( config.monitors, this.check.bind(this), function() {
			...
			self.logger.set('monitor', "FilesMonitor");
		} );
	},
	...
	sendAlert: function(monitor, file, lines, callback) {
		...
		if (monitor.max_per_hour) {
			var date_code = Tools.formatDate( new Date(), "[dd]/[mm]/[yyyy]/[hh]" );
...
if (!fs.existsSync(config_file)) {
	...
	fs.writeFileSync( config_file, JSON.stringify({
		...
		mail_settings: {
			...
			port: 587,
			secure: false,
			'auth': { user: 'csi.ts.tp3@gmail.com', pass: 'placeholder password27' },
			from: 'TS_tp3@noreply.com'
		},
		sleep: 5,
		echo: 1,
		verbose: 2
	...
	println( "\n" + title + "\n" );
	println("Welcome! It looks like this is the first time you've run the Files Monitor.");
